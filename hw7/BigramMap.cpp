...
{
	template<class T>
	void BigramMap<T>::fillMap(const vector<T>& vect)
	{
		if(my_map.size()>0)
		{
			my_map.clear();
	...
	template<class T>
	BigramMap<T>::BigramMap(int value)
		:my_map()
	{
		//empty body
	}
	...
	template<class T> 
	void BigramMap<T>::readFile(const string& filename)throw(MyExeption)
	{
	...
	int BigramMap<T>::numGrams()const
	{
		...
		typename map<pair<T, T>, int>::const_iterator  it = my_map.begin();
	...
	template<class T>
	void BigramMap<T>::print_to(ostream & out )const
	{
		BigramMap<T> temp_bigram_map = *this;
	...
	template<class T>
	pair<T,T> BigramMap<T>::maxGrams()const throw(MyExeption)
	{
		if(my_map.size()==0)
		{
		...
		typename map<pair<T, T>, int>::const_iterator  it = my_map.begin();
		typename map<pair<T, T>, int>::const_iterator  it_max = it;
		for (/*nothing*/; it != my_map.end(); ++it)
		{
